...
def kalman_2state_fit(g, q1, q2, alpha, rho, lam, b_labs, r_labs, b_cyt, r_cyt):
    ...
    lip = g["lip_score"].to_numpy(dtype=float) if "lip_score" in g.columns else None
    met1 = g["met_pca_1"].to_numpy(dtype=float)
    met4 = g["met_pca_4"].to_numpy(dtype=float)
    met10 = g["met_pca_10"].to_numpy(dtype=float)
    ...
    for i in range(n):
        if i > 0:
            ...
            met1_i = met1[i] if np.isfinite(met1[i]) else 0.0
            met4_i = met4[i] if np.isfinite(met4[i]) else 0.0
            met10_i = met10[i] if np.isfinite(met10[i]) else 0.0
            imm_i = x[1] if np.isfinite(x[1]) else 0.0
            lip_i = lip[i] if (lip is not None and np.isfinite(lip[i])) else 0.0

...
def kalman_2chan_fit(g, q0, alpha, b_labs, r_labs, b_cyt, r_cyt):
...
def main():
    ...
    q_grid = np.array([0.20,0.30,0.40]) ** 2
    b_labs = 0.25
    r_labs = 0.80 ** 2

    alpha_grid = np.linspace(0.80, 1.10, 7)
    b_grid = np.linspace(0.40, 1.00, 7)
    r_grid = np.linspace(0.30, 0.90, 7) ** 2
        ...
        for alpha in alpha_grid:
            for b_cyt in b_grid:
                for r_cyt in r_grid:
                    ...
                    for g in groups:
                        fit = kalman_2chan_fit(g, q0, float(alpha), b_labs, r_labs, float(b_cyt), float(r_cyt))
                        if fit is None:
                            continue
    ...
    print("✅ Tuned 2-channel + learned alpha fit complete")
    print(f"Saved: {OUT}")
    print(f"Params saved: {OUT_PARAMS}")
    print(f"Chosen alpha: {best['alpha']}")
